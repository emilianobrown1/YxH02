...
async def start_tictactoe(client, message):
    ...
    games[chat_id] = {
        ...
        "player2_name": message.reply_to_message.from_user.first_name,
        "board": create_board(),
        "turn": player1_id,
    }

    await message.reply(
        f"🎮 Tic-Tac-Toe started between {games[chat_id]['player1_name']} (❌) and {games[chat_id]['player2_name']} (⭕)\n\n"
        f"{games[chat_id]['player1_name']}, it's your turn!",
...
async def handle_move(client, query: CallbackQuery):
    ...
    if winner:
        if winner == "draw":
            ...
            winner_name = game["player1_name"] if winner == "❌" else game["player2_name"]
            winner_user = await get_user(winner_id)
            await winner_user.add_tictactoe_win()
            await add_tictactoe_game(winner_id, "win")
            await query.message.edit_text(f"🎉 {winner_name} won the game! +100,000 💎")
        del games[chat_id]
        return

    game["turn"] = game["player2"] if game["turn"] == game["player1"] else game["player1"]
    current_turn_name = game["player1_name"] if game["turn"] == game["player1"] else game["player2_name"]

    await query.message.edit_text(
        f"🎮 Tic-Tac-Toe Game\n\n{current_turn_name}, it's your turn!",
...
async def forfeit_game(client, message):
    ...
    if message.from_user.id not in [game["player1"], game["player2"]]:
        return await message.reply("You're not part of this game!")
    ...
    await message.reply(f"😢 {message.from_user.first_name} forfeited! {opponent_name} wins! 🎉")